  ...
  <div class="mx-auto max-w-4xl space-y-10 px-4 py-12 sm:px-6 lg:px-0">
      ...
      <div class="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
        <div class="space-y-4">
          <div class="flex items-center gap-4">
            @if ($profile->avatar_url)
              <img src="{{ $profile->avatar_url }}" alt="{{ __('Avatar of :name', ['name' => $user->name]) }}" class="h-20 w-20 rounded-3xl object-cover shadow-lg" />
            @else
            ...
            @endif
            <div>
              <p class="text-xs font-semibold uppercase tracking-[0.28em] text-muted-foreground">{{ __('Public profile') }}</p>
              <h1 class="text-3xl font-semibold text-foreground">{{ $profile->username ?? $user->name }}</h1>
              <p class="text-sm text-muted-foreground">{{ __('Member since :date', ['date' => $user->created_at?->translatedFormat('F Y')]) }}</p>
            </div>
          </div>
          @if ($profile->bio)
        ...
        <div class="grid w-full max-w-xs grid-cols-2 gap-3 text-center text-sm sm:w-auto">
          <div class="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <p class="text-xs uppercase tracking-[0.28em] text-muted-foreground">{{ __('Current streak') }}</p>
            <p class="mt-2 text-2xl font-semibold text-foreground">{{ $streaks['current'] }}</p>
          </div>
          <div class="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <p class="text-xs uppercase tracking-[0.28em] text-muted-foreground">{{ __('Best streak') }}</p>
            <p class="mt-2 text-2xl font-semibold text-foreground">{{ $streaks['longest'] }}</p>
          </div>
          <div class="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <p class="text-xs uppercase tracking-[0.28em] text-muted-foreground">{{ __('Public logs') }}</p>
            <p class="mt-2 text-2xl font-semibold text-foreground">{{ $stats['public_logs'] }}</p>
          </div>
          <div class="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <p class="text-xs uppercase tracking-[0.28em] text-muted-foreground">{{ __('Hours logged') }}</p>
    ...
    @if (! empty($recentBadges))
      <section class="rounded-3xl border border-emerald-200/60 bg-emerald-500/10 p-6 shadow-sm">
        <h2 class="text-sm font-semibold uppercase tracking-[0.28em] text-emerald-700">{{ __('Recent badges') }}</h2>
    ...
    <section class="space-y-6 rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
      <div class="flex items-center justify-between gap-4">
        <div>
          <h2 class="text-lg font-semibold text-foreground">{{ __('Latest public logs') }}</h2>
          <p class="text-xs text-muted-foreground">{{ __('Latest entries shared by :name.', ['name' => $profile->username ?? $user->name]) }}</p>
        </div>
        <a
          href="{{ route('home') }}"
      ...
      @if ($publicLogs->isEmpty())
        <p class="text-sm text-muted-foreground">{{ __('No public log yet.') }}</p>
      @else
        <div class="space-y-4">
    ...
    @if (! empty($stats['projects']))
      <section class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-foreground">{{ __('Most worked-on projects') }}</h2>
                    ...
                    @foreach ($stats['projects'] as $project => $count)
                        <div class="rounded-2xl border border-border/70 bg-background/80 p-4">
                          ...
                          <p class="text-xs text-muted-foreground">
                            {{ trans_choice(':count session|:count sessions', $count, ['count' => $count]) }}
                          </p>
            </div>
          @endforeach
        </div>

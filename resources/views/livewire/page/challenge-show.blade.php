...
<div class="mx-auto max-w-6xl space-y-12 px-4 py-10 sm:px-6 lg:px-0">
        ...
        <div class="relative grid gap-10 p-8 lg:grid-cols-[1.25fr_0.75fr] lg:p-10">
          ...
          <span
            ...
            <div class="relative space-y-4 rounded-3xl border border-border/60 bg-card/90 p-6 shadow-xl">
                ...
                @if (! $isOwner)
                    <div
                        class="rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-xs text-muted-foreground">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="flex items-center gap-2">
                                ...
                                @if ($item['user']->id === $run->owner_id)
                                    <span
                                        class="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">Owner</span>
                                @endif
                            </div>
        ...
        <article class="space-y-6">
            @if ($isOwner)
                <div class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
                    ...
                    <p class="text-xs text-muted-foreground">Une personne par run actif. Les invités existants doivent
                        terminer leur challenge avant d'accepter.</p>
                    ...
                    @if ($lastInviteLink)
                        <p class="mt-3 text-xs text-muted-foreground">
                            Lien généré :
                            <x-filament::link class="font-mono text-primary"
                                              href="{{ $lastInviteLink }}">{{ Str::limit($lastInviteLink, 40) }}</x-filament::link>
                        </p>
                    @endif
            ...
            @else
                <div class="rounded-3xl border border-border/60 bg-card/90 p-6 text-sm text-muted-foreground shadow-sm">
                    ...
                    <p class="mt-2">Tu participes à ce challenge. Utilise le journal quotidien pour documenter tes
                        shipments et surveille ton badge streak.</p>
                </div>
            @endif
            ...
            <div class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    @if (! $isOwner)
                        <x-filament::button color="gray" size="sm" wire:click="leave"
                                            wire:confirm="Quitter ce challenge ?">
    ...
    <section class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
        ...
        <p class="text-xs text-muted-foreground">Visualise ta progression sur {{ $targetDays }} jours. Les cases pleines
            représentent tes entrées complétées.</p>

                        ...
                        <div class="space-y-3 rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
                            ...
                            <ul class="space-y-2 text-sm">
                                @foreach ($pendingInvitations as $invitation)
                                    <li
                                        class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-background px-3 py-2">
                                        <div>
...
<div class="mx-auto max-w-6xl space-y-12 px-4 sm:px-6 ">
    ...
    <section
        ...
        'owner' => $run->owner->name,
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div
                        class="rounded-2xl border border-border/70 bg-gradient-to-br from-amber-500/10 via-orange-400/10 to-primary/10 p-4">
                        <p class="text-xs uppercase tracking-widest text-muted-foreground">{{ __('Current streak') }}
                        </p>
                        <div class="mt-3 flex items-center gap-4">
                            <div
                                class="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 via-orange-500 to-rose-400 text-3xl shadow-inner shadow-amber-500/40">
                                🔥
                            </div>
                            <div>
                        ...
                        <div class="mt-4 flex items-center gap-1">
                            ...
                            @if ($currentStreak > 7)
                                <span
                                    class="rounded-full bg-amber-500/15 px-2 py-0.5 text-[11px] font-semibold text-amber-600">
                                    +{{ $currentStreak - 7 }}
                                </span>
                            @endif
                        </div>
    ...
    <section class="grid gap-6 lg:grid-cols-[1.4fr_0.6fr]">
        <article class="space-y-6 rounded-3xl border border-border/60 bg-card/90 p-3 shadow-sm">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p
                            class="mt-1 whitespace-pre-line rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm">
                            {{ $todayEntry->notes ?: '—' }}
                        </p>
                    </div>
            ...
            @if ($todayEntry)
            <div class="rounded-3xl border border-border/60 bg-background/90 p-6 shadow-sm" @if($shouldPollAi)
            wire:poll.7s="pollAiPanel" @endif>
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center" x-data="{
                    copied: null,
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <p class="text-xs uppercase tracking-widest text-muted-foreground">{{ __('Share drafts') }}
                            </p>
                            ...
                            @if ($aiPanel['status'] === 'ready' && ($linkedinTemplate || $xTemplate))
                                <div class="mt-1 space-y-3">
                                    @if ($linkedinTemplate)
                                        <div>
                                            <p
                                                class="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground/70">
                                                LinkedIn</p>
                                            <pre
            ...
            <section class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm" id="share-section">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <dd>{{ $summary['streak'] ?? 0 }}
                            {{ trans_choice(':count day|:count days', $summary['streak'] ?? 0, ['count' => $summary['streak'] ?? 0]) }}
                        </dd>
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    @forelse ($history as $entry)
                        <button type="button" wire:click="setDate('{{ $entry['date'] ?? $challengeDate }}')"
                            ...
                            <div class="mt-1 flex items-center justify-between text-xs text-muted-foreground">
                                ...
                                <span>{{ count($entry['projects']) }}
                                    {{ trans_choice(':count project|:count projects', count($entry['projects']), ['count' => count($entry['projects'])]) }}</span>
                            </div>
                        </button>
                    @empty
                ...
                <div class="mt-3 space-y-2 text-sm">
                    @forelse ($projectBreakdown as $project)
                        <div
                            ...
                            <span class="text-xs text-muted-foreground">{{ $project['count'] }}
                                {{ trans_choice(':count day|:count days', $project['count'], ['count' => $project['count']]) }}</span>
                        </div>
                    @empty
                        <p class="text-xs text-muted-foreground">

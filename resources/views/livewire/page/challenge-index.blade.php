    ...
    <div class="relative grid gap-10 p-8 lg:grid-cols-[1.2fr_0.8fr] lg:p-10">
      <div class="space-y-6">
        ...
        <div class="flex flex-wrap gap-3">
          <a wire:navigate href="{{ $ctaPrimary['route'] }}"
          ...
          @if ($ctaSecondarySupportsNavigate)
            <a wire:navigate href="{{ $ctaSecondary['route'] }}"
        ...
        <div class="grid gap-4 sm:grid-cols-3">
          <div class="rounded-2xl border border-border/70 bg-card/90 p-4">
            ...
            <p class="text-xs text-muted-foreground">{{ \Illuminate\Support\Str::plural(__('challenge'), $ownedCount) }}
              {{ __('owned') }}</p>
          </div>
          <div class="rounded-2xl border border-border/70 bg-card/90 p-4">
          ...
          <div class="rounded-2xl border border-border/70 bg-card/90 p-4">
            ...
            <p class="mt-2 text-2xl font-semibold text-foreground">
              {{ $activeRun ? __('Active') : __('Ready to launch') }}</p>
  ...
  <section class="grid gap-6 lg:grid-cols-2">
    <article class="rounded-3xl border border-border/60 bg-card/90 p-6 shadow-sm">
      ...
      <div class="mt-4 space-y-3">
        ...
        <div
          class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border {{ $isActive ? 'border-primary/60' : 'border-border/70' }} bg-background/80 px-4 py-3 text-sm">
          <div>
    ...
    'status' => strtoupper($run->status),
          ...
          <div class="flex items-center gap-2">
            @if ($isActive)
              <span
                class="rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-600">{{ __('Active run') }}</span>
            @elseif ($run->status === 'active')
              <span
                class="rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-600">{{ __('Active') }}</span>
            @endif
            <a wire:navigate href="{{ route('challenges.show', $run) }}"
        ...
        @empty
        <div
          class="rounded-2xl border border-dashed border-border/70 bg-background/80 px-4 py-6 text-center text-sm text-muted-foreground">

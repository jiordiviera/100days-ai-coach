  ...
  @if ($repository)
    <div class="mt-5 space-y-3 text-sm">
      ...
      <div class="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span class="font-medium text-foreground">{{ \Illuminate\Support\Str::ucfirst($repository->visibility) }}</span>
      </div>
    </div>
  @elseif ($isReady && ! $errorMessage)
    <form wire:submit.prevent="createRepository" class="mt-5 space-y-4">
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <span wire:loading wire:target="createRepository" class="h-4 w-4 animate-spin rounded-full border-2 border-primary-foreground/70 border-t-transparent"></span>
          <span>{{ $isProcessing ? 'Création...' : 'Créer le repository' }}</span>
        </x-filament::button>
        <p class="text-xs text-muted-foreground">Le template {{ config('services.github.template.repository') }} sera cloné sur le compte sélectionné.</p>
      </div>
    </form>
  @endif
</div>
